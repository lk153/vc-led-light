            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="text-left px-6 py-4 font-semibold text-slate-600">{t.orderNumber}</th>
                  <th className="text-left px-6 py-4 font-semibold text-slate-600">{t.customer}</th>
                  <th className="text-left px-6 py-4 font-semibold text-slate-600">{t.date}</th>
                  <th className="text-left px-6 py-4 font-semibold text-slate-600">{t.status}</th>
                  <th className="text-right px-6 py-4 font-semibold text-slate-600">{t.total}</th>
...
function OrderRow({
  ...
  return (
    <>
      ...
      {expanded && (
        <tr>
          <td colSpan={6} className="bg-slate-50 px-6 py-6">
                ...
                <div className="space-y-2">
                  {order.items.map((item) =>
                    editingItemId === item.id ? (
                      ...
                      <div key={item.id} className="flex items-center gap-3 bg-white p-3 rounded-lg border border-slate-100">
                        {item.imageUrl ? (
                          <div
                            className="w-12 h-12 shrink-0 rounded bg-slate-100 bg-center bg-no-repeat bg-cover"
                            style={{ backgroundImage: `url("${item.imageUrl}")` }}
